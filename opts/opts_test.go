...
func TestExclusiveAB_BC_CA_DE(t *testing.T) {
	...
	s.AddDep("b", "g")
	selected.Toggle("g")
	if !Eq(selected.StringSlice(), []string{"a", "c", "b", "f", "g"}) {
		t.Errorf("Toggle expected {a, c, b, f, g} got %s", selected.StringSlice())
	}

	selected.Toggle("b")
	if !Eq(selected.StringSlice(), []string{"f"}) {
		t.Errorf("Toggle expected {f} got %s", selected.StringSlice())
	}
}
